...
if ((Test-Path -Path $CacheGroups) -eq $true)
{
	$null = (Remove-Item -Path $CacheGroups -Force -ErrorAction Continue)
}

$null = (New-Item -ItemType File -Path $CacheGroups -Force -ErrorAction Continue)
$global:CacheDistroGroupsFile = "$env:TEMP\MUCCacheDistroGroups.txt"

if ((Test-Path -Path $CacheDistroGroupsFile) -eq $true)
{
	$null = (Remove-Item -Path $CacheDistroGroupsFile -Force -ErrorAction Continue)
}

$null = (New-Item -ItemType File -Path $CacheDistroGroupsFile -Force)
$global:CacheSecurityGroupsFile = "$env:TEMP\MUCCacheSecurityGroups.txt"

if ((Test-Path -Path $CacheSecurityGroupsFile) -eq $true)
{
	$null = (Remove-Item -Path $CacheSecurityGroupsFile -Force -ErrorAction Continue)
}

$null = (New-Item -ItemType File -Path $CacheSecurityGroupsFile -Force -ErrorAction Continue)
$CachedLicenses = "$env:TEMP\MUCLicenses.txt"

if ((Test-Path -Path $CachedLicenses) -eq $true)
{
	$null = (Remove-Item -Path $CachedLicenses -Force -ErrorAction Continue)
}

#region Office365SKUs
$Sku = @{
...
#endregion Office365SKUs

[int]$Global:click = 0
$ADSyncModule = (Get-Module -ListAvailable -Name 'ADSync' -ErrorAction Continue)
...
function Add-Logs
{
	...
	$null = (Get-datesortable)
	$console.Text += "[$global:datesortable] - $text
   "
	$null = (Set-Alias -Name alogs -Value Add-Logs -Description 'Add content to the TextBoxLogs')
	$null = (Set-Alias -Name Add-Log -Value Add-Logs -Description 'Add content to the TextBoxLogs')
}
#endregion Add Logs
